...
def generateExample3_tf(input_size, output_shape):
    # Set a seed (that way you get the same values when rerunning the function)
    np.random.seed(10)
    ...
    # output layer, fully connected
    l3 = np.random.rand(1, output_shape)
    l3b = np.random.rand(1)
...
def generateExample1():
    ...
    # First hidden layer, one kernels
    l1k1 = np.random.rand(3, 3)
    l1b1 = np.random.rand(1)
...
if __name__ == "__main__":
    ...
    if example == 'example2':
        ...
        l1k1, l1k2, l1b1, l1b2, l3, l3b, input, output = generateExample3_tf(8, 18)
        img = np.expand_dims(input, axis=2)
        l1k1 = l1k1.reshape(3, 3, 1, 1)
        l1k2 = l1k2.reshape(3, 3, 1, 1)
        w1 = w1 = np.concatenate((l1k1, l1k2), axis=3)
        w1_b = [w1, np.array([l1b1[0], l1b2[0]])]

        # w3_b = [np.transpose(l3),l3b]
        w3_b = np.array([np.concatenate([l3.flatten(), l3b])])
        print(w1_b[0].shape, w3_b[0].shape)
        model = NeuralNetwork(inputSize=img.shape, loss=0, lr=learning_rate)
        parameters = {'num_filter': 2, 'filter_size': 3, 'activation': 1, 'weights': w1_b}
        model.addLayer(category='conv', parameters=parameters)
        parameters = {'filter_size': 2}
        model.addLayer(category='maxpool', parameters=parameters)
        ...
        print("Output before: \n")
        print(model.output, '\n')
        print("layer 1 kernel 1 weights and bias\n")
        weight1 = model.layers[0].weights
        print(weight1[0][:, :, 0, 0], weight1[1][0], '\n')
        print("layer 1 kernel 2 weights and bias")
        print(weight1[0][:, :, 0, 1], weight1[1][1], '\n')
        print("fully connected layer weights and bias")
        weight3 = model.layers[3].weights
        print(weight3[0][:-1], weight3[0][-1], '\n')
        ...
        # w3_b = [np.transpose(l3),l3b]
        w3_b = np.array([np.concatenate([l2.flatten(), l2b])])
        print(w1_b[0].shape, w3_b[0].shape)
        model = NeuralNetwork(inputSize=img.shape, loss=0, lr=learning_rate)
        parameters = {'num_filter': 1, 'filter_size': 3, 'activation': 1, 'weights': w1_b}
        model.addLayer(category='conv', parameters=parameters)
        ...
        print(weight1[0][:, :, 0, 0], weight1[1][0], '\n')
        print("fully connected layer weights and bias\n")
        weight2 = model.layers[2].weights
        print(weight2[0][:-1], weight2[0][-1], '\n')
        model.train(img, output, epochs=1)
        print("Output after: \n")
        print(model.output, '\n')
